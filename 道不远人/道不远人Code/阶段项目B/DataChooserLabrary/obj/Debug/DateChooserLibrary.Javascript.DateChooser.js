...
function DateChooser(elementID,name,dateFormat,minDate,maxDate,fxShow,fxHide){
    ...
    if(minDate) 
        this._minDate = minDate.formatParse(this._dateFormat);//Date.parseLocale(minDate,this._dateFormat);
    if(maxDate)
        this._maxDate = maxDate.formatParse(this._dateFormat);//Date.parseLocale(maxDate,this._dateFormat);
    this._fxShow = fxShow;
    if(!this._fxShow ||this._fxShow == 'None')
        this._fxShow = 'show()';
    this._fxHide = fxHide;
    if(!this._fxHide || this._fxHide == 'None')
        this._fxHide = 'hide()';
}
//初始化控件
...
DateChooser.monthSrc = '<%= WebResource("DateChooserLibrary.Images.month.png") %>';
DateChooser.prevHotSrc = '<%= WebResource("DateChooserLibrary.Images.bPrev-hot.png") %>';
DateChooser.prevSrc = '<%= WebResource("DateChooserLibrary.Images.bPrev.png") %>';
DateChooser.prevDownSrc = '<%= WebResource("DateChooserLibrary.Images.bPrev-down.png") %>';
DateChooser.nextHotSrc = '<%= WebResource("DateChooserLibrary.Images.bNext-hot.png") %>';
DateChooser.nextSrc = '<%= WebResource("DateChooserLibrary.Images.bNext.png") %>';
DateChooser.nextDownSrc = '<%= WebResource("DateChooserLibrary.Images.bNext-down.png") %>';
DateChooser.dropDownSrc = '<%= WebResource("DateChooserLibrary.Images.edit_0.gif") %>';
DateChooser.addSrc = '<%= WebResource("DateChooserLibrary.Images.edit_1.gif") %>';
DateChooser.reduceSrc = '<%= WebResource("DateChooserLibrary.Images.edit_2.gif") %>';
...
DateChooser.prototype._initiateComponents = function (){
    ...
    if(this._input.onchange){
        var fn = this._input.onchange;
        var obj = this;
        this._input.onchange = function(){
            var date = obj._validate(this.value);
            if(date){
                this.value = date.format(obj._dateFormat)
                fn();
            }
        }
    }
    ...
    //利用EventUtil.CreateEventHandler()创建委托的好处在于，事件触发时，
    //响应函数的执行上下文(this)不会被替换成事件源控件或window对象
    //点击控件之外时，关闭选择面板     
    $(document.body).click(EventUtil.CreateEventHandler(this,this._bodyOnClick));
    $(this._element).click(EventUtil.CreateEventHandler(this,this._elementOnClick));
    $(this._input).click(EventUtil.CreateEventHandler(this,this._inputOnClick))
        .focus(EventUtil.CreateEventHandler(this,this._inputOnClick))
        .keyup(EventUtil.CreateEventHandler(this,this._inputOnKeyUp));
    
    //如果不支持TextRange，如FireFox，则可以按下按钮后，以加速度递增或递减日期
    if(!document.selection || !document.selection.createRange){
        $(".calendarAddButton",this._element)
            .mousedown(EventUtil.CreateEventHandler(this,this.setIntervalAdd))
            .mouseup(EventUtil.CreateEventHandler(this,this.clearIntervalPart))
            .mouseout(EventUtil.CreateEventHandler(this,this.clearIntervalPart));
        $(".calendarReduceButton",this._element)
            .mousedown(EventUtil.CreateEventHandler(this,this.setIntervalReduce))
            .mouseup(EventUtil.CreateEventHandler(this,this.clearIntervalPart))
            .mouseout(EventUtil.CreateEventHandler(this,this.clearIntervalPart));
    }
    //初始化月视图
    ...
    //卸载页面时，注销事件绑定，避免内存泄露
    var _obj = this;
    $(window).unload(function document$unload(){
        $(document.body).unbind();
        $(_obj._element).unbind();
        $(_obj._input).unbind();
        $("a",_obj._element).unbind();
        $("img",_obj._element).unbind();
        _obj._element = null;
        _obj._input = null;
        _obj = null;
    });  
...
DateChooser.prototype._inputOnKeyUp = function(){
    ...
    if(date){
        ...
        this._correctError = window.setTimeout(function(){
            obj._input.value = date.format(obj._dateFormat);
...
DateChooser.prototype.showPanel = function(){
    ...
    var location = $('.calendarInputView',this._element).offset({border:true,padding:true,scroll:true});
    location.top += $('.calendarInputView',this._element).height();
    $('.calendarView',this._element).css({'left':location.left,'top':location.top});
    //最后弹的日期面板在最上面
    DateChooser._zIndex ++;
    //使用特效
...
DateChooser.prototype._showMonthView = function (){
    ...
    $(".calendarTitle",this._element).html(this._showDate.format('yyyy MM'))
...
DateChooser.prototype._showYearView = function(){
    ...
    $(".calendarTitle",this._element).html(this._showDate.format('yyyy'))
        .attr("href",'javascript:foo();');
    var obj = this;
    //每个月是不是在可选范围之类
    $('.calendarYearTable a',this._element).each(function(i){
...
DateChooser.prototype._adjustWidth = function(){
    ///调整选择面板的宽度
    if($('.calendarYearView',this._element).css('display') != 'none'){
        $('.calendarYearView',this._element).css('width',$('.calendarYearView',this._element).attr('scrollWidth'));
    }
    else{
        $('.calendarMonthView',this._element).css('width',$('.calendarMonthView',this._element).attr('scrollWidth'));
    }
}
//填充月视图内容
...
DateChooser.prototype._validate = function(strDate){
    ...
    if(strDate && strDate.length >0){
        try{
            date = strDate.formatParse(this._dateFormat);
...
DateChooser.prototype._synchronizeToInput = function(){
    $(this._input).val(new Date(this._currentDate).format(this._dateFormat));
}
...
DateChooser.prototype.selectDate = function select$date(date){
    this.set_currentDate(date);
    if(this._input.onchange){
        //$(this._input).change(); //fix jquery1.1.3 bug
        if(this._input.fireEvent){
