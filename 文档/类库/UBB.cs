...
//            sDetail = sDetail.Replace("\r\n", "<BR>");
//            #endregion
//            #region 处[b][/b] [i][/i] [u][/u] [p][/p] [sup][/sup] [sub][/sub] [center][/center]标记
//            r = new Regex(@"(\[sub\])([ \S\t]*?)(\[\/sub\])", RegexOptions.IgnoreCase);
//            for (m = r.Match(sDetail); m.Success; m = m.NextMatch())
//            {
//                sDetail = sDetail.Replace(m.Groups[0].ToString(), "<SUB>" + m.Groups[2].ToString() + "</SUB>");
...
//            #region 处[flash=x][/flash]标记
//            r = new Regex(@"<a href=([\S\t]*?) TARGET=_blank><IMG SRC=/images/swf.gif border=0 alt=点击开新窗口欣赏该FLASH动画!> [全屏欣赏]</a><br><br>
...
//            #region 处[IMG][/IMG]标记
//            r = new Regex(@"<br><a onfocus=this.blur() href=(http|https|ftp):\/\/([ \S\t]*?) target=_blank><IMG SRC=(http|https|ftp):\/\/([ \S\t]*?) border=0 alt=按此在新窗口浏览图片 onload=javascript:if(screen.width-333<this.width)this.width=screen.width-333></a>", RegexOptions.IgnoreCase);
//            for (m = r.Match(sDetail); m.Success; m = m.NextMatch())
//            {
